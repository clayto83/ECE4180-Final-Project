...
void wave_player::play(FILE *wavefile)
{
        ...
        extern bool playing;  // Linked in main, determines play or pause
        // extern bool changed;  // Linked in main, determines if user skips or goes back song
        extern int vol;       // Linked in main, volume control
        extern Mutex mutex;   // Linked in main, keeps mbed from crashing when attempting to skip songs
...
//
        for (slice=0;slice<num_slices;slice+=1) {
          // Added code to enable pausing 
          while (!playing){
            DAC_on = 0;
            wait(0.1);
          // Added code to enable changinging songs
          // if (changed) break;
          }
          // Added code to enable changinging songs
          // if (changed){
